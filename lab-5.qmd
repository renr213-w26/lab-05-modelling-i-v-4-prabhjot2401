...
```{r}
bind_rows(
  tidy(dt_model) |> filter(term == "divergence_time") |> mutate(model = "dt_model"),
  tidy(dist_model) |> filter(term == "distance") |> mutate(model = "dist_model"),
  tidy(BM_model) |> filter(term == "BMdiff") |> mutate(model = "BM_model"),
  tidy(prec_model) |> filter(term == "precdiff") |> mutate(model = "prec_model")
) |> 
  select(model, term, estimate)
```

The slope estimates from each model are: 

$$
\begin{aligned}
\hat{\beta}_{dt} &= \text{<r signif(coef(dt_model)[2], 3)>} \\\\
\hat{\beta}_{dist} &= \text{<r signif(coef(dist_model)[2], 3)>} \\\\
\hat{\beta}_{BM} &= \text{<r signif(coef(BM_model)[2], 3)>} \\\\
\hat{\beta}_{prec} &= \text{<r signif(coef(prec_model)[2], 3)>}
\end{aligned}
$$
