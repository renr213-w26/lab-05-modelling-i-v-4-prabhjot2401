...
```{r}
bind_rows(
  tidy(dt_model) %>% filter(term == "divergence_time") %>%
    mutate(model = "$\\hat{\\beta}_{\\text{divergence\\_time}}$~(dt\\_model)"),
  tidy(dist_model) %>% filter(term == "distance") %>%
    mutate(model = "$\\hat{\\beta}_{\\text{distance}}$~(dist\\_model)"),
  tidy(BM_model) %>% filter(term == "BMdiff") %>%
    mutate(model = "$\\hat{\\beta}_{\\text{BMdiff}}$~(BM\\_model)"),
  tidy(prec_model) %>% filter(term == "precdiff") %>%
    mutate(model = "$\\hat{\\beta}_{\\text{precdiff}}$~(prec\\_model)")
) %>%
